...
// El semestre anterior será el mismo que el semestre actual
$anio_semestre_anterior = $anio_anterior . '-' . $semestre_actual;
...
echo "</tr>
    <tr>
        ...
        <th class='difference' title='Diferencia en  promedio puntos asignados entre los dos periodos' style='text-align: center !important;'>X&#x0305;.Ptos</th>
           ...
           <th class='difference' title='Diferencia en  valores proyectados entre los dos periodos' style='text-align: center !important;'>\$Proyect</th>
        ";
...
while ($row = $resultado->fetch_assoc()) {
 ...
 echo "<tr>
    ...
    <td class='$textoColor' style='text-align: left; font-size: 0.9em; position: relative;'>
        <form action='depto_comparativo.php' method='POST' class='d-inline'>
          ...
          <input type='hidden' name='anio_semestre_anterior' value='" . htmlspecialchars($anio_semestre_anterior) . "'>
                    ...
                    <input type='hidden' name='envia' value='rcec'>
    ...
    <td class='current-period' style='text-align: center !important; vertical-align: middle !important;'>" . 
     ($row['total_actual'] != 0 ? number_format($row['puntos_actual'] / $row['total_actual'], 2) : "0.00") . 
     "</td>";
...
$valor_redondeado = round($valor_final / 1000000); // Redondea al entero más cercano
$valor_formateado = '$' . $valor_redondeado . 'M';

echo "<td class='current-period' 
 ...
 echo "
    ...
    <td class='previous-period' style='text-align: center !important; vertical-align: middle !important;'>".($row['total_anterior'] > 0 ? number_format($row['puntos_anterior']/$row['total_anterior'], 2) : '0.00')."</td>";
...
$valor_en_millones = round($valor_final / 1000000); // Redondeo al entero más cercano
$valor_formateado = '$' . $valor_en_millones . 'M';

echo "<td class='previous-period' style='text-align: center !important; vertical-align: middle !important;'>" . 
     $valor_formateado . 
     "</td>";
...
echo "<td class='diff-period $color_clase' 
     ...
     {$signo}$" . number_format($diferencia_millones, 0, ',', '.') . "M
     </td>";
