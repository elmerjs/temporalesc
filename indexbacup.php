...
<!DOCTYPE html>
    ...
    <body>
        <h1>Solicitudes de Contratación Temporales</h1>
        ...
        <form action="guardar.php" method="POST">
            ...
            <div id="facultad-group" class="button-group">
                ...
                // Obtener facultades
                $result = $conn->query("SELECT PK_FAC, nombre_fac_minb FROM facultad");
                while ($row = $result->fetch_assoc()) {
                    echo '<button type="button" class="facultad-btn" data-value="' . $row['PK_FAC'] . '">' . $row['nombre_fac_minb'] . '</button>';
                }
                $conn->close();
                ?>
            </div>
