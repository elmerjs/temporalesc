...
$consulta_novedad_tipos = "SELECT DISTINCT novedad
                            ...
                            AND novedad IS NOT NULL AND novedad != ''
                                AND estado_depto = 'PENDIENTE'
                            "; // Asegurar que novedad no sea nula o vacía
...
while ($row_novedad_tipo = $resultado_novedad_tipos->fetch_assoc()) {
    ...
    $consultat = "SELECT solicitudes_working_copy.*,
                   ...
                   AND novedad = '$novedad_actual'
                     AND estado_depto = 'PENDIENTE'
                   ORDER BY solicitudes_working_copy.nombre ASC"; // Ordenar por nombre del profesor
    ...
    // --- INICIO DE CAMBIO ---
    $unique_observations = []; // Almacenará: ['texto_observacion' => [indice1, indice2, ...]]
    $temp_results = []; // Array temporal para guardar los resultados de la consulta y poder iterar dos veces
    $cont_for_observations = 0; // Contador para el número de cada ítem en la lista de solicitudes
    // --- FIN DE CAMBIO ---
