...
$consulta_depto_unic = "SELECT  *  
                  ...
                  WHERE fk_depto_dp = '$departamento_id' AND periodo = '$anio_semestre' limit 1;";

$resultadodepto_unic = $con->query($consulta_depto_unic);

if (!$resultadodepto_unic) {
    die('Error en la consulta: ' . $con->error);
}



while ($rowdepto_unic = $resultadodepto_unic->fetch_assoc()) {
         $fk_fac = $rowdepto_unic['FK_FAC']; // Reemplaza con la columna correcta

     $num_oficio = $rowdepto_unic['num_oficio_depto']; // Reemplaza con la columna correcta
    $elaboro = $rowdepto_unic['proyecta']; // Reemplaza con la columna correcta
    $nombre_fac = $rowdepto_unic['nombre_fac_min']; // Reemplaza con la columna correcta
    $acta = $rowdepto_unic['dp_acta_periodo']; // Reemplaza con la columna correcta
       $fecha_acta = $rowdepto_unic['dp_fecha_acta']; // Reemplaza con la columna correcta
...
$actab = $acta . ' del ' . $fecha_acta_b;
    $fecha_oficio = $rowdepto_unic['dp_fecha_envio']; // Reemplaza con la columna correcta
    $folios = isset($rowdepto_unic['dp_folios']) && trim($rowdepto_unic['dp_folios']) !== '' 
              ? trim($rowdepto_unic['dp_folios']) 
              : 0;
}



$decano = obtenerDecano($fk_fac);
...
$header->addImage($imgencabezado, array(
    ...
    'marginTop' => -284, // Subir la imagen para compensar el espacio de margen superior de 1 cm
...
while ($row = $resultadot->fetch_assoc()) {
    ...
    $table->addRow();
    $table->addCell(400, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
        ->addText($cont, $cellTextStyle, $paragraphStyle);
    $table->addCell(1200, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
        ->addText(utf8_decode($row['cedula'] ?: ''), $cellTextStyle, $paragraphStyle);
    $table->addCell(4000, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
        ->addText(utf8_decode($row['nombre'] ?: ''), $cellTextStyle, $paragraphStyle);

    if ($tipo_d == "Ocasional") {
        $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
            ->addText(utf8_decode($row['tipo_dedicacion'] ?: ''), $cellTextStyle, $paragraphStyle);
        $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
            ->addText(utf8_decode($row['tipo_dedicacion_r'] ?: ''), $cellTextStyle, $paragraphStyle);
    } elseif ($tipo_d == "Catedra") {
        $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
            ->addText(utf8_decode($row['horas'] ?: ''), $cellTextStyle, $paragraphStyle);
        $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
            ->addText(utf8_decode($row['horas_r'] ?: ''), $cellTextStyle, $paragraphStyle);
    }
$table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
...
$section->addText(
    'Dedicación/hr = Dedicación / Horas, Pop. = Popayán, Reg = Regionalización, Anx.Nuev = Anexa Nuevo, Actl.Antg = Actualiza Antiguo, TC = Tiempo Completo, MT = Medio Tiempo', 
    $fontStyleSmall, 
    $paragraphStyleSmall
);
    $section->addTextBreak(); // Inserta un salto de línea
