...
if (!empty($cedulas_faltantes)) {
    ...
    echo "<script>
        alert('$mensaje');
        window.location.href = 'indexsolicitud.php';
    </script>";
    exit; // Detener el flujo si hay cédulas faltantes
}
            
            
                                    // Insertar los datos si no hay cédulas duplicadas
            for ($i = 0; $i < $num_docentes; $i++) {
                $cedula = $_POST['cedula'][$i];
                $nombre = $_POST['nombre'][$i];
                $tipo_dedicacion = isset($_POST['tipo_dedicacion'][$i]) ? $_POST['tipo_dedicacion'][$i] : null;
                $tipo_dedicacion_r = isset($_POST['tipo_dedicacion_r'][$i]) ? $_POST['tipo_dedicacion_r'][$i] : null;
              $horas_r = isset($_POST['horas_r'][$i]) ? (int)$_POST['horas_r'][$i] : 0; // Conversión a entero
$horas = isset($_POST['horas'][$i]) ? (int)$_POST['horas'][$i] : 0;       // Conversión a entero
...
if (($horas + $horas_r) > 12) {
    ...
    exit;
                ...
                $anexa_hv_docente_nuevo = $_POST['anexa_hv_docente_nuevo'][$i];
                $actualiza_hv_antiguo = $_POST['actualiza_hv_antiguo'][$i];
