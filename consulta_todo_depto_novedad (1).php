...
function fetchData($conn, $working_table_name, $anio_semestre, $departamento_id, $novedad_type) {
    ...
    $sql = "SELECT
                ...
                novedad, s_observacion, tipo_reemplazo
...
$sql = "
SELECT
    -- Seleccionamos todos los campos de solicitudes_working_copy (sw)
    sw.id_solicitud,
    sw.anio_semestre,
    sw.facultad_id,
    sw.departamento_id,
...
WHERE
    ...
    -- *** CAMBIO CLAVE AQUÍ: Ahora muestra tanto PENDIENTE como ENVIADO por depto. ***
...
SELECT
    -- Seleccionamos los campos de la tabla original 'solicitudes' (s)
    s.id_solicitud,
    s.anio_semestre,
    s.facultad_id,
    s.departamento_id,
    ...
    s.otra_experiencia,
    COALESCE(swc_s.estado_depto, 'PENDIENTE') AS estado_depto, -- Asumimos 'PENDIENTE' si no hay SWC activa
    swc_s.oficio_depto,
    swc_s.fecha_envio_depto,
    swc_s.aprobador_depto_id,
    COALESCE(swc_s.estado_facultad, 'PENDIENTE') AS estado_facultad,
    swc_s.observacion_facultad,
    swc_s.fecha_aprobacion_facultad,
    swc_s.aprobador_facultad_id,
    COALESCE(swc_s.estado_vra, 'PENDIENTE') AS estado_vra,
    swc_s.observacion_vra,
    swc_s.fecha_aprobacion_vra,
    swc_s.aprobador_vra_id,
    swc_s.fk_id_solicitud_original,
    f.nombre_fac_minb AS nombre_facultad,
    d.depto_nom_propio AS nombre_departamento,
    s.novedad AS tipo_novedad
FROM
    solicitudes s
JOIN
...
LEFT JOIN
    solicitudes_working_copy swc_s ON (swc_s.fk_id_solicitud_original = s.id_solicitud)
WHERE
    ...
    AND NOT EXISTS (
        SELECT 1
        FROM solicitudes_working_copy sw_live
        WHERE
            sw_live.fk_id_solicitud_original = s.id_solicitud
            -- *** CAMBIO CLAVE AQUÍ: Refleja que una working_copy activa puede estar PENDIENTE o ENVIADO por depto. ***
            AND (sw_live.estado_depto = 'ENVIADO' OR sw_live.estado_depto = 'PENDIENTE')
            AND sw_live.estado_facultad <> 'RECHAZADO'
            AND sw_live.estado_vra <> 'RECHAZADO'
            AND NOT (sw_live.estado_facultad = 'APROBADO' AND sw_live.estado_vra = 'APROBADO')
    )
ORDER BY nombre ASC;
";
...
if ($tipo_usuario == 3) { // Asumiendo que $tipo_usuario ya está definido y 3 es el rol que puede eliminar
    echo "
...
<div class='modal fade' id='actaModal' tabindex='-1' aria-labelledby='actaModalLabel' aria-hidden='true'>
    <div class='modal-dialog modal-lg'>
        <div class='modal-content'>
            ...
            <div class='modal-body'>
                <form id='actaForm' action='for_45b.php' method='GET'>
                   <input type='hidden' name='modal_solicitud_id' id='modal_id_solicitud'>
                    ...
                    <input type='hidden' name='modal_cedula_prof' id='modal_cedula_profesor'>
                        ...
                        <p class="mb-1"><strong>ID Solicitud:</strong> <span id="display_id_solicitud"></span></p>
                        <p class="mb-1"><strong>Cédula Profesor:</strong> <span id="display_cedula_profesor"></span></p>
                        <p class="mb-1"><strong>ID Departamento:</strong> <span id="display_departamento_id"></span></p>
                        <p class="mb-0"><strong>Período (Año/Semestre):</strong> <span id="display_anio_semestre"></span></p>
                    </div>
                    <div class='row mb-4'>
                        <div class='col-md-6'>
                            <div class="mb-3">
...
<h4 class="text-center mb-4 display-8 fw-bold text-dark">Gestión de Novedades de Docentes</h4>

    <!-- Sección para Novedades: Adicionar -->
    <div class="card mb-5 shadow-lg">
        <div class="card-header bg-primary text-white">
            <h4 class="mb-0"><i class="fas fa-plus-circle me-3"></i>Novedades: Adicionar</h4>
        </div>
                       ...
                       <table id="tableAdicionar" class="table table-striped table-hover table-bordered dt-responsive nowrap" style="width:100%">
                        <thead>
                            <tr>
                                ...
                                // Esto es para la tabla actual ($data_adicionar)
                                $hasOcasional = false;
                                $hasCatedra = false;
                                foreach ($data_adicionar as $row_check) { // Usamos $row_check para evitar conflicto con $row del foreach principal
                                ...
                                ?>
                                <th>Observación</th>
                                <th>Deshacer</th> <!-- Nueva columna de Acción -->
                            </tr>
                        </thead>
                        <tbody>
...
<?php endif; ?>
                    ...
                    <td title="<?= htmlspecialchars($row['s_observacion']) ?>">
                        <?= htmlspecialchars(mb_strimwidth($row['s_observacion'], 0, 14, '…')) ?>
                                    ...
                                    <td class="td-simple">
                                        <?php if ($tipo_usuario == 3): // Asumiendo que $tipo_usuario está definido ?>
                                           <form action='deshacer_novedad_mod.php' method='POST' style='display:inline;'>
                        <input type='hidden' name='id_solicitud' value='<?= htmlspecialchars($row["id_solicitud"]) ?>'>
                        <input type='hidden' name='anio_semestre' value='<?= htmlspecialchars($anio_semestre) ?>'>
                        <input type='hidden' name='departamento_id' value='<?= htmlspecialchars($departamento_id) ?>'>
                        <input type='hidden' name='novedad_a_deshacer' value='adicionar'>
                        <button type='submit' title='Deshacer Adición' style="padding: 0; margin: 0; border: none; background: none; line-height: 1; font-size: 14px;">
    ...
    <!-- Sección para Novedades: Eliminar -->
    <div class="card mb-5 shadow-lg">
    ...
    <?php elseif (($row['sede'] ?? '') === 'Regionalización'): ?>
        ...
        <td style="display:none;"></td>
    <?php else: ?>
        <!-- Si la sede no es Popayán ni Regionalización, o no está definida para Ocasional -->
        <td><?= htmlspecialchars($row['tipo_dedicacion']) ?></td>
        <td style="display:none;"></td>
    <?php endif; ?>
<?php elseif (($row['tipo_docente'] ?? '') === 'Catedra'): ?>
    ...
    <?php elseif (($row['sede'] ?? '') === 'Popayán-Regionalización'): ?>
        ...
        <td><?= htmlspecialchars($row['horas']) . '; ' . htmlspecialchars($row['horas_r']) ?></td>
    <?php else: ?>
        <!-- Si la sede no es Popayán, Regionalización, ni Popayán-Regionalización, o no está definida para Catedra -->
        <td style="display:none;"></td>
        <td><?= htmlspecialchars($row['horas']) ?></td>
    <?php endif; ?>
<?php else: ?>
    <!-- Si tipo_docente no es Ocasional ni Catedra -->
    <td><?= htmlspecialchars($row['tipo_dedicacion']) ?></td>
    <td><?= htmlspecialchars($row['horas']) ?></td>
<?php endif; ?>
                        ...
                        <?php if ($tipo_usuario == 3): ?>
                            <form action='deshacer_novedad_mod.php' method='POST' style='display:inline;'>
                                ...
                                <button type='submit' title='Deshacer Eliminación' style="padding: 0; margin: 0; border: none; background: none; line-height: 1; font-size: 14px;">
                                    <i class='fas fa-undo'></i>
                                </button>
                            </form>
                        <?php endif; ?>
    ...
    <!-- Sección para Novedades: Modificar -->
    <div class="card mb-5 shadow-lg">
...
<td title="<?= htmlspecialchars($row['s_observacion']) ?>">
    <?= htmlspecialchars(mb_strimwidth($row['s_observacion'], 0, 14, '…')) ?>
</td>            <td class="td-simple" style="padding: 0; vertical-align: middle;">
                            <?php if ($tipo_usuario == 3): ?>
                                <form action='deshacer_novedad_mod.php' method='POST' style='display:inline;'>
                                    <input type='hidden' name='id_solicitud' value='<?= htmlspecialchars($row["id_solicitud"]) ?>'>
                                    <input type='hidden' name='anio_semestre' value='<?= htmlspecialchars($anio_semestre) ?>'>
                                    ...
                                    <button type='submit' title='Deshacer Modificación' style="padding: 0; margin: 0; border: none; background: none; line-height: 1; font-size: 14px;">
                                        <i class='fas fa-undo'></i>
                                    </button>
                                </form>
                            <?php endif; ?>
