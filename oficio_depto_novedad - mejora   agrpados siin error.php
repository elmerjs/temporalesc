...
while ($row_cambio = $result_cambio_vinculacion->fetch_assoc()) {
    ...
    $cedulas_cambio_vinculacion[] = $cedula;
    $cambio_vinculacion_data[] = $row_cambio;
}
$cedulas_excluir_str = !empty($cedulas_cambio_vinculacion) ? 
    "'" . implode("','", $cedulas_cambio_vinculacion) . "'" : '';

// ===========================================
// SECCIÓN: CAMBIO DE VINCULACIÓN (NUEVA LÓGICA)
// ===========================================
if (!empty($cambio_vinculacion_data)) {
    ...
    foreach ($cambio_vinculacion_data as $cambio) {
        // 1. Mostrar observación si existe
        if (!empty($cambio['observacion_adicionar'])) {
            $section->addText(
                htmlspecialchars($cambio['observacion_adicionar']), 
                $observationTextStyle, 
                $paragraphStyleLeft
            );
        }
        
        // 2. Construir texto narrativo del cambio
        $nombre_profesor = htmlspecialchars($cambio['nombre_adicionar']);
        $tipo_anterior = htmlspecialchars($cambio['tipo_docente_eliminar']);
        $tipo_nuevo = htmlspecialchars($cambio['tipo_docente_adicionar']);
        
        // Detalles vinculación anterior
        $vinculacion_anterior = "";
        if ($tipo_anterior === "Ocasional") {
            $dedicacion_pop = $cambio['dedicacion_eliminar'];
            $dedicacion_reg = $cambio['dedicacion_r_eliminar'];
            $parts = [];
            if (!empty($dedicacion_pop)) {
                $parts[] = str_replace(['MT', 'TC'], ['Medio Tiempo', 'Tiempo Completo'], $dedicacion_pop) . ' (Popayán)';
            }
            if (!empty($dedicacion_reg)) {
                $parts[] = str_replace(['MT', 'TC'], ['Medio Tiempo', 'Tiempo Completo'], $dedicacion_reg) . ' (Regionalización)';
            }
            $vinculacion_anterior = implode(" y ", $parts);
        } elseif ($tipo_anterior === "Catedra") {
            $horas_pop = ($cambio['horas_eliminar'] > 0) ? $cambio['horas_eliminar'] : '';
            $horas_reg = ($cambio['horas_r_eliminar'] > 0) ? $cambio['horas_r_eliminar'] : '';
            $parts = [];
            if (!empty($horas_pop)) {
                $parts[] = $horas_pop . "hr (Popayán)";
            }
            if (!empty($horas_reg)) {
                $parts[] = $horas_reg . "hr (Regionalización)";
            }
            $vinculacion_anterior = implode(" y ", $parts);
        }
        
        // Detalles nueva vinculación
        $nueva_vinculacion = "";
        if ($tipo_nuevo === "Ocasional") {
            $dedicacion_pop = $cambio['dedicacion_adicionar'];
            $dedicacion_reg = $cambio['dedicacion_r_adicionar'];
            $parts = [];
            if (!empty($dedicacion_pop)) {
                $parts[] = str_replace(['MT', 'TC'], ['Medio Tiempo', 'Tiempo Completo'], $dedicacion_pop) . ' (Popayán)';
            }
            ...
            $nueva_vinculacion = implode(" y ", $parts);
        } elseif ($tipo_nuevo === "Catedra") {
            $horas_pop = ($cambio['horas_adicionar'] > 0) ? $cambio['horas_adicionar'] : '';
            $horas_reg = ($cambio['horas_r_adicionar'] > 0) ? $cambio['horas_r_adicionar'] : '';
            $parts = [];
            if (!empty($horas_pop)) {
                $parts[] = $horas_pop . "hr (Popayán)";
            }
        ...
        // Texto descriptivo
        $texto_narrativo = "Profesor: {$nombre_profesor} - Cambia de {$tipo_anterior}";
        if (!empty($vinculacion_anterior)) {
            $texto_narrativo .= " - " . $vinculacion_anterior;
        }
        $texto_narrativo .= " a {$tipo_nuevo}";
        if (!empty($nueva_vinculacion)) {
            $texto_narrativo .= " - " . $nueva_vinculacion;
        }

        $section->addText($texto_narrativo, $observationTextStyle, $paragraphStyleLeft);
        ...
        $phpWord->addTableStyle('CambioVinculacionNewTable', $styleTable);
        $table = $section->addTable('CambioVinculacionNewTable');
        ...
        $table->addCell(1200, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($cambio['cedula']), $cellTextStyle, $paragraphStyle);
        $table->addCell(4000, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($nombre_profesor), $cellTextStyle, $paragraphStyle);
        
        // Dedicación/Horas
        if ($tipo_nuevo === "Ocasional") {
            $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($cambio['dedicacion_adicionar']), $cellTextStyle, $paragraphStyle);
            $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($cambio['dedicacion_r_adicionar']), $cellTextStyle, $paragraphStyle);
        } else { // Catedra
            $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($cambio['horas_adicionar']), $cellTextStyle, $paragraphStyle);
            $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($cambio['horas_r_adicionar']), $cellTextStyle, $paragraphStyle);
        }
        
        // Hoja de vida
        $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(
            mb_strtoupper(htmlspecialchars($cambio['anexa_hv_docente_nuevo'])), 
            $cellTextStyle, 
            $paragraphStyle
        );
        $table->addCell(350, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(
            mb_strtoupper(htmlspecialchars($cambio['actualiza_hv_antiguo'])), 
            $cellTextStyle, 
            $paragraphStyle
        );
        
        // Tipo docente
        $table->addCell(1000, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(
            htmlspecialchars($tipo_nuevo), 
            $cellTextStyle, 
            $paragraphStyle
        );
        ...
        // 4. Actualizar estado de ambas solicitudes
        ...
        if ($stmt) {
            $stmt->bind_param('ii', 
                $cambio['id_solicitud_eliminar'], 
                $cambio['id_solicitud_adicionar']
            );
            $stmt->execute();
            $stmt->close();
        } else {
            // Manejar error de prepared statement
            // echo "Error preparando la consulta de actualización de cambio de vinculación: " . $con->error;
        }
    }
}
...
while ($row_novedad_tipo = $resultado_novedad_tipos->fetch_assoc()) {
    ...
    foreach ($temp_results as $row) {
        ...
        $facultad_email = $row['email_facultad'] ?? 'ejurado@unicauca.edu.co'; // Usar el de la BD si existe, sino el fallback
