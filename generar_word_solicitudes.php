...
setlocale(LC_TIME, 'es_CO.utf8', 'es_CO', 'es_ES.utf8', 'es_ES', 'spanish');
...
// Consultar solicitudes aprobadas por facultad y pendientes por VRA
$sql = "
    SELECT
        sw.id_solicitud,
        sw.cedula,
...
if ($stmt) {
    $stmt->bind_param("si", $anio_semestre, $id_facultad);
    $stmt->execute();
...
if (empty($solicitudes)) {
    die("No se encontraron solicitudes APROBADAS por Facultad y PENDIENTES por VRA para el periodo y facultad seleccionados.");
}
...
$phpWord->addFontStyle('normalSize11', ['size' => 11, 'lang' => 'es-ES']); // Añade 'lang'
$phpWord->addFontStyle('normalSize9', ['size' => 9, 'lang' => 'es-ES']); // Añade 'lang'
...
$phpWord->addParagraphStyle('left_no_space', ['alignment' => Jc::LEFT, 'spaceAfter' => 0]);
...
$cellTextStyle = ['size' => 9, 'bold' => false, 'lang' => 'es-ES']; // <-- IMPORTANTE
$cellTextStyleb = ['size' => 9, 'bold' => true, 'lang' => 'es-ES'];  // <-- IMPORTANTE
...
$fontStyleb = ['bold' => true, 'size' => 10, 'lang' => 'es-ES'];    // <-- IMPORTANTE
$observationTextStyle = ['size' => 9, 'italic' => true, 'lang' => 'es-ES']; // <-- IMPORTANTE
...
$timestamp_fecha_oficio = strtotime($fecha_oficio); // Convertir la fecha a un timestamp
$section->addText('Popayán, ' . strftime('%d de %B de %Y', $timestamp_fecha_oficio), 'normalSize11', 'left');
...
// Destinatario (Vicerrectoría Académica)
$section->addText('Señor(a)', 'normalSize11', 'left_no_space'); // Cambiado a 'left_no_space'
$section->addText('Vicerrector/a Académico/a', 'normalSize11', 'left_no_space');
$section->addText('Universidad del Cauca', 'normalSize11', 'left_no_space'); // Cambiado a 'left_no_space'
$section->addText('Ciudad.', 'normalSize11', 'left_no_space'); // Cambiado a 'left_no_space'
$section->addTextBreak(1); // Esta línea agregará un salto de línea después de "Ciudad."

$section->addText(
    'Asunto: Novedades de Contratos Docentes ' . $anio_semestre . '.',
    'normalSize11', // Usa el estilo de fuente 'normalSize11' que ya tiene el 'lang'
    'left'          // Usa el estilo de párrafo 'left'
);
...
foreach ($grouped_solicitudes as $departamento_nombre => $novedades_por_depto) {
    // Aquí se asume que el estilo de fuente para 'Departamento de...' ya tiene lang: 'es-ES'
    $section->addText('Departamento de ' . htmlspecialchars($departamento_nombre), ['bold' => true, 'size' => 11, 'lang' => 'es-ES']); 
    $section->addTextBreak(0);

    foreach ($novedades_por_depto as $novedad_tipo => $solicitudes_por_novedad) {
        $novedad_mostrar = ucfirst($novedad_tipo);
        // '$fontStyleb' debe incluir 'lang' => 'es-ES' en su definición
        ...
        if (!empty($grouped_observations[$departamento_nombre][$novedad_tipo])) {
            ...
            // '$observationTextStyle' debe incluir 'lang' => 'es-ES' en su definición
        ...
        // Nº
        // '$cellTextStyle' debe incluir 'lang' => 'es-ES' en su definición
        ...
        // Nombre
        // '$cellTextStyle' debe incluir 'lang' => 'es-ES' en su definición
        $row->addCell(3900, array_merge($headerCellStyle, array('vMerge' => 'restart')))->addText('Nombre', $cellTextStyle, $paragraphStyle);
        
        // Dedicación/hr
        // '$cellTextStyle' debe incluir 'lang' => 'es-ES' en su definición
        $row->addCell(1100, array_merge($headerCellStyle, array('alignment' => Jc::CENTER, 'gridSpan' => 2, 'vMerge' => 'restart')))->addText('Dedic/hr', $cellTextStyle, $paragraphStyle);
        
        // Hoja de vida
        // '$cellTextStyle' debe incluir 'lang' => 'es-ES' en su definición
        $row->addCell(1100, array_merge($headerCellStyle, array('alignment' => Jc::CENTER, 'gridSpan' => 2, 'vMerge' => 'restart')))->addText('H.Vida', $cellTextStyle, $paragraphStyle);
        ...
        $row->addCell(3900, array('vMerge' => 'continue', 'borderSize' => 6, 'borderColor' => '999999')); // Nombre
        
        // Sub-encabezados de Dedicación/hr
        // '$cellTextStyleb' debe incluir 'lang' => 'es-ES' en su definición
        $row->addCell(550, $headerCellStyle)->addText('Pop', $cellTextStyle, $paragraphStyle);
        $row->addCell(550, $headerCellStyle)->addText('Reg', $cellTextStyle, $paragraphStyle);
        
        // Sub-encabezados de Hoja de vida
        // '$cellTextStyleb' debe incluir 'lang' => 'es-ES' en su definición
        $row->addCell(550, $headerCellStyle)->addText('Nuev', $cellTextStyle, $paragraphStyle);
        $row->addCell(550, $headerCellStyle)->addText('Antg', $cellTextStyle, $paragraphStyle);
        ...
        foreach ($solicitudes_por_novedad as $sol) {
            ...
            $table->addCell(3900, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText($full_nombre, $cellTextStyle, $paragraphStyle);

            // Columnas de Dedicación/horas según el tipo de docente
            if ($sol['tipo_docente'] == "Ocasional") {
                // '$cellTextStyle' debe incluir 'lang' => 'es-ES' en su definición
                $table->addCell(550, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($sol['tipo_dedicacion'] ?: ''), $cellTextStyle, $paragraphStyle);
                $table->addCell(550, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($sol['tipo_dedicacion_r'] ?: ''), $cellTextStyle, $paragraphStyle);
            } elseif ($sol['tipo_docente'] == "Catedra") {
                // '$cellTextStyle' debe incluir 'lang' => 'es-ES' en su definición
                $table->addCell(550, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($sol['horas'] ?: ''), $cellTextStyle, $paragraphStyle);
                $table->addCell(550, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText(htmlspecialchars($sol['horas_r'] ?: ''), $cellTextStyle, $paragraphStyle);
            } else {
                // '$cellTextStyle' debe incluir 'lang' => 'es-ES' en su definición
                $table->addCell(550, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText('', $cellTextStyle, $paragraphStyle);
                $table->addCell(550, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])->addText('', $cellTextStyle, $paragraphStyle);
            }

            // Columnas de Hoja de Vida
            // '$cellTextStyle' debe incluir 'lang' => 'es-ES' en su definición
            $table->addCell(550, ['borderSize' => 1, 'marginTop' => 0, 'marginBottom' => 0])
...
$section->addText(
    $decano_nombre,
    ['bold' => true, 'size' => 11], 'left_no_space'
);
$section->addText(
